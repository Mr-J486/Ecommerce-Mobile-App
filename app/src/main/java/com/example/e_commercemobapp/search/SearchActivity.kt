...
//                Toast.makeText(this, "Failed loading products", Toast.LENGTH_SHORT).show()
//            }
//    }
//
//    // ---------------------------------------------------
...
//                    CategoryAdapter(list) { selectedCategory ->
//                        if (selectedCategory == "All") {
//                            filterProducts("")
//                            searchInput.setText("")
//                        } else {
...
import com.example.e_commercemobapp.model.Category
import com.example.e_commercemobapp.model.Product
import com.example.e_commercemobapp.search.adapters.CategoryAdapter
import com.example.e_commercemobapp.search.adapters.ProductAdapter
import com.example.e_commercemobapp.ui.auth.LoginActivity
import com.google.firebase.auth.FirebaseAuth
...
class SearchActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        loadCategories()    // ALWAYS LOAD FIRST
        loadProducts()      // THEN PRODUCTS (depends on categories)
    ...
    private fun loadCategories() {
        db.collection("categories")
            ...
            .addOnSuccessListener { result ->
                ...
                list.add(Category("all", "All", "Show all"))
                ...
                categoryRecycler.adapter =
                    CategoryAdapter(list) { cat ->

                        if (cat.name.lowercase() == "all") {
                            searchInput.setText("")
                            filterProducts("")
                        } else {
                            searchInput.setText(cat.name)
    ...
    private fun loadProducts() {
        db.collection("products")
            ...
            .addOnSuccessListener { result ->
                ...
                adapter = ProductAdapter(filteredList, categoryNames, this)
                recyclerView.adapter = adapter
            }
    }
    ...
    private fun filterProducts(query: String) {
        ...
        if (q.isEmpty()) {
            ...
            filteredList.addAll(
                productList.filter {
                    it.name.lowercase().contains(q) ||
                            categoryNames[it.categoryId]?.lowercase()?.contains(q) == true ||
                            it.barcode.contains(q)
                }
            )
        }
